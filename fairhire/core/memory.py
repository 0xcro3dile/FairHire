...
class Memory:
  ...
  def store(self, key: str, data: dict[str, Any], ttl: int = 86400 * 90) -> None:
    self.client.setex(key, ttl, orjson.dumps(data, option=orjson.OPT_SERIALIZE_NUMPY).decode("utf-8"))
  ...
  def store_batch(self, items: list[tuple[str, dict[str, Any]]], ttl: int = 86400 * 90) -> None:
    ...
    for key, data in items:
      pipe.setex(key, ttl, orjson.dumps(data, option=orjson.OPT_SERIALIZE_NUMPY).decode("utf-8"))
    pipe.execute()
  ...
  def recall_batch(self, keys: list[str]) -> dict[str, dict[str, Any] | None]:
    ...
    return {
      key: orjson.loads(val) if val else None
      for key, val in zip(keys, results, strict=True)
    }
